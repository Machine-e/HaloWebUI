...
async def compact_chat_branch(request, user, chat: Any, model_id: str, models: dict) -> dict:
    ...
    messages = get_message_list(messages_map, current_id) or []
    messages, previous_summary = _apply_latest_summary_checkpoint(messages)
    if len(messages) <= 2:
...
async def _load_config() -> dict:
    ...
    return {
        ...
        "token_threshold": int(
            values.get("chat.context_compaction.token_threshold", 80000) or 80000
        ),
...
def _resolve_token_threshold(global_threshold: int, metadata: dict) -> int:
    configured_threshold = _parse_positive_int(
        (metadata.get("params") or {}).get("compact_token_threshold")
    )
