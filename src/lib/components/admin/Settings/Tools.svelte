<script lang="ts">
	...
	const normalizeNativeToolsConfig = (value: Record<string, any> | null | undefined) => ({
		...
		ENABLE_LIST_KNOWLEDGE_BASES:
			value?.ENABLE_LIST_KNOWLEDGE_BASES ?? defaultNativeToolsConfig.ENABLE_LIST_KNOWLEDGE_BASES,
		...
		ENABLE_QUERY_KNOWLEDGE_FILES:
			value?.ENABLE_QUERY_KNOWLEDGE_FILES ?? defaultNativeToolsConfig.ENABLE_QUERY_KNOWLEDGE_FILES,
		...
		ENABLE_IMAGE_GENERATION_TOOL:
			value?.ENABLE_IMAGE_GENERATION_TOOL ?? defaultNativeToolsConfig.ENABLE_IMAGE_GENERATION_TOOL,
		ENABLE_IMAGE_EDIT: value?.ENABLE_IMAGE_EDIT ?? defaultNativeToolsConfig.ENABLE_IMAGE_EDIT,
		ENABLE_MEMORY_TOOLS: value?.ENABLE_MEMORY_TOOLS ?? defaultNativeToolsConfig.ENABLE_MEMORY_TOOLS,
	...
	const normalizeMCPRuntimeCapabilities = (value: any): MCPRuntimeCapabilities => {
		...
		for (const [command, capability] of Object.entries(value?.commands ?? {})) {
			commands[command] = {
				...
				message:
					typeof (capability as any)?.message === 'string' ? (capability as any).message : null
			};
		}
	...
	const getMCPHeaderCount = (server: any): number => Object.keys(server?.headers ?? {}).length;
	...
	const canShareMCPServer = (server: any): string | null => {
		if (
			(server?.transport_type ?? 'http') === 'http' &&
			(server?.auth_type ?? 'none') === 'session'
		) {
	...
	const submitHandler = async () => {
		...
		try {
			...
			const okOpenAPI = canUseDirectToolServers ? await saveOpenAPIServers({ silent: true }) : true;
		...
		<div class="max-w-6xl mx-auto space-y-6">
				...
				<div class="@container flex flex-col gap-5">
					<div class="flex flex-col gap-4">
						...
						>
							<button
								type="button"
								class={`flex min-w-0 items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'native' ? 'bg-white text-gray-900 shadow-[0_1px_3px_rgba(15,23,42,0.08)] dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:bg-white/50 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-800/50 dark:hover:text-gray-200'}`}
								on:click={() => {
									selectedTab = 'native';
								}}
							>
							...
							{#if canUseDirectToolServers}
								<button
									type="button"
									class={`flex min-w-0 items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'mcp' ? 'bg-white text-gray-900 shadow-[0_1px_3px_rgba(15,23,42,0.08)] dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:bg-white/50 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-800/50 dark:hover:text-gray-200'}`}
									on:click={() => {
										selectedTab = 'mcp';
									}}
								>
							...
							<button
								type="button"
								class={`flex min-w-0 items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'workspace' ? 'bg-white text-gray-900 shadow-[0_1px_3px_rgba(15,23,42,0.08)] dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:bg-white/50 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-800/50 dark:hover:text-gray-200'}`}
								on:click={() => {
									selectedTab = 'workspace';
								}}
							>
							...
							{#if canUseDirectToolServers}
								<button
									type="button"
									class={`flex min-w-0 items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-all ${selectedTab === 'openapi' ? 'bg-white text-gray-900 shadow-[0_1px_3px_rgba(15,23,42,0.08)] dark:bg-gray-800 dark:text-white' : 'text-gray-500 hover:bg-white/50 hover:text-gray-800 dark:text-gray-400 dark:hover:bg-gray-800/50 dark:hover:text-gray-200'}`}
									on:click={() => {
										selectedTab = 'openapi';
									}}
								>
			...
			{#if selectedTab === 'native'}
				...
				>
					...
					<div class="glass-item p-4">
						...
						<div class="text-xs text-gray-500 dark:text-gray-400">
							{#if nativeToolsConfig.TOOL_CALLING_MODE === 'default'}
								{$i18n.t(
									'Compatibility mode uses prompt-based tool orchestration, works with a wider range of models, but may be slower.'
								)}
							{:else if nativeToolsConfig.TOOL_CALLING_MODE === 'off'}
								{$i18n.t(
									'Off mode disables tool calling while keeping your selected MCP / OpenAPI tools configured.'
								)}
							{:else}
								{$i18n.t(
									"Native mode uses the model's built-in tool-calling capabilities for faster, more reliable multi-step operations. Requires a strong tool-capable model."
								)}
							{/if}
						</div>
					</div>
					...
					<!-- Web Tools -->
					<div class="glass-item p-4">
						<div class="text-sm font-medium text-gray-600 dark:text-gray-300 mb-2.5">
							{$i18n.t('网络工具')}
						</div>
						<div class="space-y-2">
							<div class="flex items-center justify-between">
								<div class="text-xs text-gray-500">
									{$i18n.t('网络搜索')}
									<code
										class="text-[11px] bg-gray-100/80 dark:bg-gray-800/60 px-1 py-0.5 rounded font-mono"
										>search_web</code
									>
								</div>
						...
						{#if mcpServers.length > 0}
							<div class="space-y-2">
								{#each mcpServers as server, idx}
									<div class="flex items-center justify-between p-3 glass-item">
										<div class="flex-1 min-w-0">
											<div class="flex items-center gap-2">
												...
												>
													{server.verified_at ? $i18n.t('已验证') : $i18n.t('未验证')}
												</span>
												{#if server.shared_id}
													<span
														class="px-1.5 py-0.5 text-xs rounded bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 shrink-0"
											...
											{#if server.verified_at}
												<div class="text-xs text-gray-400 dark:text-gray-500 mt-0.5 ml-4">
													{$i18n.t('上次验证于')}
													{formatVerifiedAt(server.verified_at)}
												</div>
											{:else}
					...
					<div class="glass-item p-4">
						...
						<div class="space-y-2 text-xs text-gray-500">
							...
							<div class="flex items-start gap-2">
								...
								>
								<span
									>{$i18n.t('适用于 x-consumer-api-key、x-api-key 或供应商要求的专用请求头')}</span
								>
							</div>
						</div>
					</div>
			...
			{:else if selectedTab === 'workspace'}
				...
				<section class="p-5 space-y-3 transition-all duration-300 glass-section">
					<!-- Security Warning -->
