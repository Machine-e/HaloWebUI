<script lang="ts">
	...
	$: dirtyConfig = ready && JSON.stringify(gatewayConfig) !== JSON.stringify(initialGatewayConfig);
	...
	const handleClientSubmit = async (event) => {
		...
		clients = clients.map((client) =>
			client.id === id ? { ...client, ...clone(data), id } : client
		);
	};
	...
	<div class="h-full space-y-6 overflow-y-auto scrollbar-hidden">
		<div class="max-w-6xl mx-auto space-y-6">
			<section class="glass-section p-5 space-y-5">
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-3">
					...
					<label class="flex items-center justify-between glass-item px-4 py-3">
						...
						<input
							...
							type="number"
							min="0"
							bind:value={gatewayConfig.default_rpm_limit}
						/>
					</label>
					...
					<label class="flex items-center justify-between glass-item px-4 py-3">
						<div>
							...
							<div class="text-xs text-gray-500 mt-1">
								提供 `{gatewayBasePath}/openai/v1/*` 入口
							</div>
						</div>
					...
					<label class="flex items-center justify-between glass-item px-4 py-3">
						<div>
							...
							<div class="text-xs text-gray-500 mt-1">
								提供 `{gatewayBasePath}/anthropic/v1/*` 入口
							</div>
						</div>
				...
				<div
					class="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 p-4 text-xs text-gray-500 space-y-1"
				>
					<div>
						OpenAI: `{gatewayBasePath}/openai/v1/models` `{gatewayBasePath}/openai/v1/chat/completions`
						`{gatewayBasePath}/openai/v1/responses`
					</div>
					<div>
						Anthropic: `{gatewayBasePath}/anthropic/v1/models` `{gatewayBasePath}/anthropic/v1/messages`
					</div>
				</div>
			</section>
			...
			<section class="glass-section p-5 space-y-5">
				...
				{#if latestCreatedKey}
					<div
						class="rounded-2xl border border-green-200 bg-green-50 dark:bg-green-950/20 dark:border-green-900/40 px-4 py-3"
					>
						<div class="text-sm font-medium text-green-700 dark:text-green-300">
							已创建新的密钥：{latestCreatedClient}
						</div>
						<div class="mt-1 break-all text-xs text-green-600 dark:text-green-400">
							{latestCreatedKey}
						</div>
						<div class="mt-2">
							<button
								type="button"
								class="px-3 py-1 rounded-full bg-green-600 text-white text-xs"
								on:click={copyLatestKey}
							>
								复制
							</button>
						</div>
				...
				<div class="space-y-3">
					{#if clients.length === 0}
						<div
							class="rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 px-4 py-8 text-center text-gray-400"
						>
							暂无外部客户端密钥
						</div>
					{:else}
						{#each clients as client}
							<div
								class="rounded-2xl border border-gray-200 dark:border-gray-700 px-4 py-4 space-y-3"
							>
								<div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
									<div class="min-w-0">
										<div class="flex items-center gap-2 flex-wrap">
											...
											<span
												class="text-[10px] rounded-full px-2 py-0.5 bg-gray-100 dark:bg-gray-800 text-gray-500"
												>{client.key_prefix}</span
											>
											{#if client.enabled}
												<span
													class="text-[10px] rounded-full px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-600"
													>启用</span
												>
											{:else}
												<span
											...
											{#if client.allow_tools}
												<span
													class="text-[10px] rounded-full px-2 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-600"
													>工具</span
												>
											{/if}
										</div>
										<div class="mt-1 text-xs text-gray-500 break-all">
											用户：{users.find((user) => user.id === client.owner_user_id)?.email ||
												client.owner_user_id}
										</div>
										<div class="mt-1 text-xs text-gray-500">
											协议：{(client.allowed_protocols ?? []).join(' / ') || '-'} · 模型数：{client
												.allowed_model_ids?.length ?? 0} · RPM：{client.rpm_limit ??
												gatewayConfig.default_rpm_limit}
										</div>
										{#if client.note}
									...
									<div class="flex flex-wrap items-center gap-2">
										<button
											type="button"
											class="px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-xs"
											on:click={() => openLogs(client)}
										>
											日志
										</button>
										<button
											type="button"
											class="px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800 text-xs"
											on:click={() => openEditClient(client)}
										>
											编辑
										</button>
										<button
											type="button"
											class="px-3 py-1 rounded-full bg-red-50 dark:bg-red-950/30 text-red-500 text-xs"
