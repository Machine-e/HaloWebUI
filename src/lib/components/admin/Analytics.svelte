<script lang="ts">
	...
	$: tabMeta = {
		overview: {
			...
			description: tr(
				'查看消息量、Token 用量和活跃度概览。',
				'View message volume, token usage, and activity at a glance.'
			),
		...
		models: {
			...
			description: tr(
				'按模型查看用量统计和趋势。',
				'Inspect usage statistics and trends by model.'
			),
		...
		users: {
			...
			description: tr(
				'按用户查看活跃度和 Token 消耗。',
				'Inspect activity and token consumption by user.'
			),
	...
	const setCleanupScope = async (scope: 'range' | 'all') => {
		...
		if (
			...
			(!cleanupPreview || cleanupPreviewError)
		) {
...
<div class="max-w-6xl mx-auto space-y-6">
					...
					<div class="flex items-start gap-3">
						...
						<div class="min-w-0">
							<div class="flex items-center gap-3">
								<div
									class="shrink-0 whitespace-nowrap text-base font-semibold text-gray-800 dark:text-gray-100"
								>
	...
	{#if activeTab === 'overview'}
		<div class="grid grid-cols-2 md:grid-cols-4 gap-3">
			{#each [{ icon: ChatBubbleOvalEllipsis, value: formatNumber(totalMessages), label: $i18n.t('Total Messages'), color: 'blue' }, { icon: Bolt, value: formatNumber(totalTokens), label: $i18n.t('Total Tokens'), color: 'amber' }, { icon: Cube, value: activeModels, label: $i18n.t('Active Models'), color: 'violet' }, { icon: UsersSolid, value: activeUsers, label: $i18n.t('Active Users'), color: 'emerald' }] as card}
				<div
				...
				>
					<div
						class="w-9 h-9 rounded-xl {cardColorMap[card.color]
							.bg} flex items-center justify-center mb-3"
					>
						<svelte:component
							this={card.icon}
							className="size-[18px] {cardColorMap[card.color].text}"
						/>
					</div>
					<div class="text-2xl font-semibold tracking-tight text-gray-900 dark:text-gray-100">
						{card.value}
					</div>
		...
		{#if dailyStats.length > 0}
			<div class="relative">
				<h3 class="text-[13px] font-medium text-gray-500 dark:text-gray-400 mb-3">
					{$i18n.t('Daily Activity')}
				</h3>
				<div class="relative">
					<div
						class="flex items-end gap-1 h-36 rounded-2xl p-3 pb-1 bg-white/70 dark:bg-gray-900/60 border border-gray-100/90 dark:border-gray-800/70 shadow-sm shadow-gray-900/[0.04] dark:shadow-black/30"
					>
		...
		<div>
			...
			<div
				class="p-5 rounded-2xl bg-white/70 dark:bg-gray-900/60 border border-gray-100/90 dark:border-gray-800/70 shadow-sm shadow-gray-900/[0.04] dark:shadow-black/30"
			>
				{#if totalTokens > 0}
					...
					<div class="flex items-center gap-3">
						...
						<div>
							...
							<div class="text-[13px] text-gray-400 dark:text-gray-500">
								...
								{totalTokens > 0 ? `(${Math.round((totalPromptTokens / totalTokens) * 100)}%)` : ''}
							</div>
						</div>
					</div>
					<div class="flex items-center gap-3">
	...
	{:else if activeTab === 'models'}
		<div
			class="rounded-2xl border border-gray-100/90 dark:border-gray-800/70 overflow-hidden bg-white/70 dark:bg-gray-900/60 shadow-sm shadow-gray-900/[0.04] dark:shadow-black/30"
		>
			<!-- Models toolbar -->
			<div
				class="flex flex-wrap items-center justify-end gap-2 px-4 py-3 border-b border-gray-100 dark:border-white/[0.06]"
			>
				<div
			...
			<table class="w-full text-sm">
				<thead>
					<tr
						class="border-b border-gray-100 dark:border-white/[0.06] bg-gray-50/80 dark:bg-gray-850/50"
					>
						{#if selectionMode}
							<th
				...
				<tbody>
					{#each filteredModelStats as stat, i}
						...
						>
							...
							<td class="px-4 py-2.5 truncate max-w-[260px]">
								<div class="flex items-center gap-1.5">
									...
									<span class={isModelActive(stat.model) ? '' : 'text-gray-400 dark:text-gray-500'}
						...
						{#if expandedModel === stat.model}
							<tr
								class="border-t border-gray-100 dark:border-white/[0.04] bg-gray-50/60 dark:bg-gray-850/30"
							>
										...
										<div class="mt-3 flex items-center gap-2 text-[11px]">
											...
											>
												...
												<div
													...
													style="width: {(stat.total_completion_tokens / stat.total_tokens) * 100}%"
												></div>
											</div>
	...
	>
		<div class="text-sm text-gray-500">
			<div class="mb-4">
				...
				<div
					class="flex rounded-xl border border-gray-200/70 dark:border-gray-800 overflow-hidden w-fit"
				>
					<button
			...
			{#if cleanupActiveModels.length > 0}
				<div
					class="mb-4 bg-yellow-500/20 text-yellow-700 dark:text-yellow-200 rounded-xl px-4 py-3"
				>
			...
			{#if cleanupScope === 'all'}
				{#if cleanupPreviewLoading}
					<div
						class="p-4 rounded-xl bg-gray-50 dark:bg-gray-900/40 border border-gray-200/70 dark:border-gray-800 animate-pulse"
					>
				...
				{:else if cleanupPreviewError}
					<div
						class="p-4 rounded-xl bg-red-50 dark:bg-red-950/20 border border-red-200/60 dark:border-red-900/30 text-red-700 dark:text-red-200 text-xs"
					>
						{cleanupPreviewError}
					</div>
				{:else if cleanupPreview?.totals}
					<div
						class="p-4 rounded-xl bg-gray-50 dark:bg-gray-900/40 border border-gray-200/70 dark:border-gray-800"
					>
						<div class="flex flex-wrap items-center gap-3 text-xs text-gray-600 dark:text-gray-300">
							...
							<span class="text-gray-400">
								{$i18n.t('Prompt Tokens')}: {formatNumber(
									cleanupPreview.totals.total_prompt_tokens
								)}
				...
				{:else}
					<div
						class="p-4 rounded-xl bg-gray-50 dark:bg-gray-900/40 border border-gray-200/70 dark:border-gray-800 text-xs text-gray-400 text-center"
					>
						{$i18n.t('No data')}
					</div>
				{/if}
			{:else}
				...
				>
					<div class="flex flex-wrap items-center gap-3 text-xs text-gray-600 dark:text-gray-300">
						...
						<span class="text-gray-400">
							{$i18n.t('Prompt Tokens')}: {formatNumber(
								cleanupRangePreview.totals.total_prompt_tokens
							)}
							&middot; {$i18n.t('Completion Tokens')}: {formatNumber(
